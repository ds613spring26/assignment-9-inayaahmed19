...
```{r}
# Filtering just for 'Emma' book:
emma <- austen |>
  filter(book == "Emma")

# Using tokenization & removing stopwords 
tidy_emma <- emma |>
...
# You can see in the below plot that the most frequent "joy"-coded word in Emma is 'friend', followed by 'hope.'
```
...
# we see 'hope' and 'happy' as the biggest words for the joy wordcloud.
```
...
# Same as the ggplot vis, we see dear as the biggest word in the wordcloud for AFINN positive vocab.
```
